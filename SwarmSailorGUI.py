...
current_Newtork_Status = Newtork_Status()
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        #Ui Tweaks
        self.setWindowTitle('Swarm M138 GUI - by SwarmSailor - ' + GUIVERSION) #Update Title
    ...
    def Button_Open_Port_click(self):
        ...
        if (port_available == False):
            ...
            self.findChild(QtWidgets.QLabel, 'data_COM_Status').setText("Error: Port Not Available!")
            try:
                self.ser.close()
            except:
                pass
        ...
        try:
            ...
            self.findChild(QtWidgets.QLabel, 'data_COM_Status').setText("Error: Failed to Open Port")
            try:
                self.ser.close()
            except:
                pass
    ...
    def Button_Close_Port_click(self):
        ...
        if (port_available == False):
            self.findChild(QtWidgets.QPlainTextEdit, 'Serial_Monitor_Display').appendPlainText("Port Is Already Closed!")
            try:
                self.ser.close()
            except:
                pass
        ...
        try:
            ...
            self.findChild(QtWidgets.QPlainTextEdit, 'Serial_Monitor_Display').appendPlainText("Error: Could Not Close The Port!")
            self.findChild(QtWidgets.QLabel, 'data_COM_Status').setText("Error: Could Not Close The Port!")
            return

        self.save_settings()
        self.findChild(QtWidgets.QPlainTextEdit, 'Serial_Monitor_Display').appendPlainText("Port is now closed")
        self.findChild(QtWidgets.QLabel, 'data_COM_Status').setText("Disconnected")
        self.findChild(QtWidgets.QLabel, 'data_network_status').setText("")

    def Button_Get_GRIB_click(self):
        dialog = QDialogGRIB()
        dialog.exec_()
        print(dialog.returnString())
    ...
    def Button_Send_Message_click(self):
        ...
        dialog.exec_()
        self.sendMessage(dialog.returnString())

    def Button_Serial_Monitor_Send_click(self):
        self.send_Serial_Command(self.findChild(QtWidgets.QLineEdit, 'Serial_Monitor_SendLine').text())
    ...
    def send_Serial_Command(self, message) -> None:
        ...
        if (port_available == False):
            self.findChild(QtWidgets.QPlainTextEdit, 'Serial_Monitor_Display').appendPlainText("Error: Port Is Not Open!")
            try:
                self.ser.close()
            except:
                pass
            ...
            while self.ser.canReadLine():
                ...
                match text[0:3]:
                    case "$RT":
                        ...
                        current_Newtork_Status.RSSI = int(array[2])
                    case '$MT':
                        array = re.split(regex, text)
    ...
    def update_com_ports(self) -> None:
        ...
        for desc, name, sys in gen_serial_ports():
            longname = desc + " (" + name + ")"
            self.findChild(QtWidgets.QComboBox, 'comboBox_PORT').addItem(longname, sys) 
    ...
    def loadHistory(self):
        ...
        if port_name is not None:
            index = self.findChild(QtWidgets.QComboBox, 'comboBox_PORT').findData(port_name)
            if index > -1:
                self.findChild(QtWidgets.QComboBox, 'comboBox_PORT').setCurrentIndex(index)
...
class QDialogMessage(QtWidgets.QDialog):
    ...
    def calculateMessage(self):
        ...
        self.findChild(QtWidgets.QLabel, 'label_Size_Calc').setText(str(len(self.return_message)) + " Chars (" + str(math.ceil(len(self.return_message)/192)) + " packets)")
...
class QDialogGRIB(QtWidgets.QDialog):
    ...
    def calculateMessage(self):
        ...
        #Model
        return_message = self.findChild(QtWidgets.QComboBox, 'comboBox_Model').currentText() + ":"
        #GPS Range
        ...
        self.findChild(QtWidgets.QLineEdit, 'lineEdit_Request').setText(return_message)

    def calc_size(self):
        self.findChild(QtWidgets.QLabel, 'data_Size_estimate').setText(str(math.ceil(len(self.findChild(QtWidgets.QLineEdit, 'lineEdit_Request').text()))) + " Chars (Max 192)")
    ...
    def change_model(self, new_model):
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Current').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_AirT').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_CAPE').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Cloud').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Pressure').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Wave').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Wind').setCheckState(False)
        self.findChild(QtWidgets.QCheckBox, 'checkBox_Wing').setCheckState(False)
        ...
        match new_model:
            case 'GFS':
                ...
                self.findChild(QtWidgets.QCheckBox, 'checkBox_Wind').setCheckState(True)
                self.findChild(QtWidgets.QCheckBox, 'checkBox_Pressure').setCheckState(True)
            case 'RTOFS':
                for x in range(1, 7):
                    self.findChild(QtWidgets.QComboBox, 'comboBox_Range').addItem(str(x)) 
                self.findChild(QtWidgets.QCheckBox, 'checkBox_Current').setCheckState(True)
                self.findChild(QtWidgets.QCheckBox, 'checkBox_Current').setEnabled(False)
